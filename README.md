...
Schema and row records carry explicit tags. Key constraints are metadata records before the row records: `~P|users|id;` declares a primary key, while `~F|posts|user_id|users|id;` declares a foreign key with the default `RESTRICT/RESTRICT` actions. Nondefault delete actions append compact delete/update tags: `~F|posts|user_id|users|id|C|R;` is `ON DELETE CASCADE`, and `|N|R` is `ON DELETE SET NULL`. An auto-incrementing key has exactly one record such as `~A|users|id|I42;`, placed after that table's primary- and foreign-key metadata. Its nonnegative high-water mark must cover every stored key for the generated column.
...
- `UPDATE` and `DELETE` do not consume the `SELECT` working budget. Their frozen targets, decoded values, exact measured-row descriptors, effective overlays, reusable expression stack, referential child and edge indexes, cascade queues, pending `SET NULL` columns, encoded replacements, and deferred sequence state use an independent accounting pass under the same private storage-working bound. Measured-row descriptors remain live from database-size preflight through row encoding; each is consumed by a bounded encoder that rejects output before it can exceed the measured length and also rejects underfilled output. Each row's overlays are released after its encoded replacement is installed. A deferred sequence descriptor is retained throughout mutation planning and charged once while evaluator frames, scan transients, frozen rows, measurements, overlays, and encoded row replacements may be live; after measurements and overlays are consumed, only the later encoded sequence replacement is checked as a transient peak. Deferred mutation state is consumed or discarded before final candidate validation begins. Exhausting this budget reports `Resource::StorageWorkingBytes`; the final authoritative candidate remains governed by `Resource::DatabaseBytes`. Mutation planning and final candidate validation use separate budget instances derived from the same bound. These logical budgets are safety rails rather than a total query or process memory cap: they exclude other planning allocations, regex-engine scratch space, the authoritative string, allocator overhead and capacity beyond conservative descriptor charges, and the candidate string itself. Logical charges include target-layout sizes, so exact boundaries can differ between 32-bit and 64-bit builds. Both `SELECT` budgets can be live at once, and a limit failure returns no partial result or mutation.
