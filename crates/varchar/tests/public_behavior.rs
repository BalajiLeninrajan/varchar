...
#[test]
fn select_resolves_all_predicates_before_physical_pattern_limits() {
    let mut database = Database::new();
    execute(
        &mut database,
        "CREATE TABLE t (id INTEGER NOT NULL, note TEXT NOT NULL)",
    ...
    let database = Database::from_string_with_limits(database.into_string(), limits)
...
fn select_working_budget_does_not_govern_update_or_delete() {
    ...
    assert_eq!(
        row_set(execute(&mut verified, "SELECT id, note FROM records")).into_rows(),
