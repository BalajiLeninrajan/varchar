...
#[test]
fn zero_match_update_at_exact_database_limit_is_a_byte_identical_no_op() {
    let mut database = Database::new();
    execute(
        &mut database,
        "CREATE TABLE ids (id INTEGER PRIMARY KEY AUTOINCREMENT)",
    ...
    let mut limited = Database::from_string_with_limits(blob.clone(), limits)
        .expect("existing database exactly fits the byte limit");

    assert_eq!(
        execute(&mut limited, "UPDATE ids SET id = 20 WHERE id = 999"),
        Outcome::Affected { rows: 0 }
    );
